...
public class MatrixCalc {
	...
	public boolean checkDotProduct(MatrixCalc m1, MatrixCalc m2){
		int[][] arr1 = m1.arr;
		int[][] arr2 = m2.arr;
	...
	public void product(MatrixCalc m){
	...
	public void transpose(){
		...
		//Loop through the arr matrix
		for(int j = 0; j < arr[0].length; j++){
			for(int i = 0; i < arr.length; i++){
				//Set new Matrix value to the transposed value
				newArr[j][i] = arr[i][j];
			}
			System.out.println();
		}
	...
	public void difference(MatrixCalc m){
		int[][] sum = m.arr;
		int[][] arr2 = m.arr;
